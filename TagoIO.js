...
const ignore_vars = ['rx_0_rf_chain', 'modulation', 'rx_0_gateway_id', 'rx_0_rssi', 'rx_0_lorasnr', 'rx_0_channel', 'rx_0_location', 'rx_0_fine_timestamp_type', 'rx_0_context', 'frequency', 'bandwidth', 'spreading_factor', 'code_rate', 'polarization_inversion', 'fCnt', 'fPort', 'deviceName', 'adr', 'dr', 'confirmedUplink', 'publishedAt', 'deviceProfileID', 'deviceProfileName', 'objectJSON', 'application_id', 'application_name', 'device_name', 'device_eui', 'dev_addr'];
...
const my_payload = payload.find(x => x.variable === "payload");
if (my_payload) {
	var bytes = hexToBytes(my_payload.value);

	lppDecode(bytes, 1).forEach(function (field) {
		payload.push({ "variable": field['name'] + '_' + field['channel'], "value": field['value'] });
	});
	// Remove raw payload.
